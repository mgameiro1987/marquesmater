...
import json, sqlite3
# V25 final pass 2: sincronização final antes do backup completo.
...
t=items[0];t['price']=12.95;t['price_display']='12,95 €';t['promo_price']=None;t['promo_enabled']=False
t['variants']=[{'name':c,'color':c,'label':'290ml / '+c,'price':12.95,'image':img,'asset_image':img} for c,img in IMAGES.items()]
t['asset_image']=IMAGES['Branco'];t['gallery_local']=list(IMAGES.values())
...
c=sqlite3.connect(DB);c.execute("DELETE FROM products WHERE brand='Soudal' AND slug<>?",(TARGET,));c.execute("UPDATE products SET price=?,price_display=?,promo_price=NULL,promo_enabled=0,image=?,gallery_json=?,variants_json=?,stock=1,updated_at=CURRENT_TIMESTAMP WHERE slug=?",(12.95,'12,95 €',IMAGES['Branco'],json.dumps(list(IMAGES.values()),ensure_ascii=False),json.dumps(t['variants'],ensure_ascii=False),TARGET));c.commit();assert c.execute("SELECT COUNT(*) FROM products WHERE brand='Soudal'").fetchone()[0]==1;c.close();print('OK')
