/* MarquesMater V10.65 — renderer direto da seleção Produtos.
   A seleção Produtos passa a usar a mesma filosofia visual de Categorias.
   Não depende da tabela antiga nem de classes da camada recovery. */
(()=>{
'use strict';
...
async function render(){
 ...
 try{
  ...
  document.getElementById('mm65c').innerHTML='<option value="">Todas as categorias</option>'+cats.map(c=>`<option value="${esc(c)}">${esc(c)}</option>`).join('');
  const grid=document.getElementById('mm65grid');
  grid.innerHTML=ps.map((p,i)=>{
   const inactive=p.active===false;
   return `<article class="mm65-card" data-q="${esc(`${p.name||''} ${p.sku||''} ${p.brand||''}`.toLowerCase())}" data-c="${esc(p.category||'')}">
    <div class="mm65-head"><img class="mm65-img" src="${esc(p.image||'images/drill.svg')}" alt=""></div>
    <div class="mm65-body"><div class="mm65-name">${esc(p.name||'Sem nome')}</div><div class="mm65-sku">SKU: ${esc(p.sku||'—')}</div>
    <div class="mm65-meta"><div class="mm65-field"><span class="mm65-label">Marca</span>${esc(p.brand||'—')}</div><div class="mm65-field"><span class="mm65-label">Preço</span><span class="mm65-price">${money(p.price)}</span></div><div class="mm65-field"><span class="mm65-label">Categoria</span>${esc(p.category||'—')}<br><small>${esc(p.subcategory||'')}</small></div><div class="mm65-field"><span class="mm65-label">Stock</span>${esc(p.stock||'Em stock')}<br><small>${Number(p.stockQty??0)} un.</small></div><div class="mm65-field"><span class="mm65-label">Estado</span><span class="mm65-pill ${inactive?'off':''}">${inactive?'Inativo':'Ativo'}</span></div></div>
    <div class="mm65-actions"><button class="btn small" data-edit="${i}">Editar</button><button class="btn small red" data-toggle="${i}">${inactive?'Ativar':'Desativar'}</button></div></div></article>`;
  }).join('')||'<div class="mm65-empty">Não existem produtos.</div>';
  ...
  document.getElementById('mm65q').oninput=filter;document.getElementById('mm65c').onchange=filter;
  document.getElementById('mm65new').onclick=()=>{const old=window.MM104&&window.MM104.go; if(old)old('products');};
 ...
 }catch(e){document.getElementById('mm65grid').innerHTML=`<div class="mm65-empty">${esc(e.message)}</div>`}
}
const originalGo=window.MM104?.go;
window.MM104={...(window.MM104||{}),go:k=>{if(k==='products')render();else if(originalGo)originalGo(k)}};
document.addEventListener('click',e=>{const b=e.target.closest('.navitem[data-section="products"]');if(!b)return;e.preventDefault();e.stopImmediatePropagation();render();document.getElementById('sidebar')?.classList.remove('open')},true);
// Se a seleção já estiver em Produtos, renderiza a vista nova. Também resiste a reconstruções de módulos antigos.
