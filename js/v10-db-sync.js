/* MarquesMater V10.7 — sincronização central + promoções + variantes + estilo Front Office */
(()=>{
  ...
  const priceChanging=p=>(variantConfig(p)?.attributes||[]).filter(a=>(a.priceMode||'none')!=='none');
  function patchStoreVariantPrice(){try{if(!window.MMStore||window.MMStore.__mm106v)return;if(typeof window.MMStore.variantPrice!=='function')return;const old=window.MMStore.variantPrice;window.MMStore.variantPrice=(p,o)=>{const v=exactVariant(p,o);if(v&&v.active!==false&&Number(v.price)>0)return Number(v.price);return old(p,o)};window.MMStore.__mm106v=true}catch(e){console.warn('V10.7 variante/preço indisponível',e)}}
  function buildVariantMaps(){try{const maps={};for(const p of (window.MARQUES_CATALOG||[])){const vs=variantConfig(p)?.variants||[],attrs=priceChanging(p);if(!vs.length||!attrs.length)continue;maps[p.sku]={};for(const a of attrs){maps[p.sku][a.name]={};for(const val of (a.values||[])){const prices=vs.filter(v=>String(v.options?.[a.name]??'')===String(val)&&v.active!==false&&Number(v.price)>0).map(v=>Number(v.price));if(prices.length)maps[p.sku][a.name][val]=Math.min(...prices)}}}window.MM_VARIANT_PRICES={...(window.MM_VARIANT_PRICES||{}),...maps}}catch(e){console.warn('V10.7 mapa de variantes indisponível',e)}}
  function loadFrontOfficeStyle(){try{if(document.getElementById('mm-v106-frontoffice-css'))return;const l=document.createElement('link');l.id='mm-v106-frontoffice-css';l.rel='stylesheet';l.href='/css/v10.6-frontoffice.css?v=107';document.head.appendChild(l)}catch(e){console.warn('V10.7 estilo Front Office indisponível',e)}}
  function loadVariantEngine(){try{if(document.getElementById('mm-v106-variant-engine'))return;const s=document.createElement('script');s.id='mm-v106-variant-engine';s.src='/js/v10.6-variant-engine.js?v=107';document.body.appendChild(s)}catch(e){console.warn('V10.7 motor de variantes indisponível',e)}}
