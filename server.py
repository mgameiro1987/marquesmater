...
PORT=int(os.environ.get('PORT','10000')); ROOT=os.path.dirname(os.path.abspath(__file__)); os.chdir(ROOT)
try: import psycopg
...
def db():
    if psycopg is None or not os.environ.get('DATABASE_URL'): raise RuntimeError('DATABASE_URL não configurado')
...
def read_json(h):
    n=int(h.headers.get('Content-Length','0') or 0); raw=h.rfile.read(n) if n else b'{}'; return json.loads(raw.decode('utf-8') or '{}')
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path=='/api/stock':
            try:
                init_db()
                with db() as c,x:
                    x.execute('SELECT sku,stock,stock_min,updated_at FROM mm_product_stock ORDER BY sku'); rows=x.fetchall()
                self.send_json(200,{'ok':True,'items':[{'sku':r[0],'stock':r[1],'stockMin':r[2],'updatedAt':r[3]} for r in rows]})
            except Exception as e:self.send_json(503,{'ok':False,'error':str(e)})
            return
        if path=='/api/stock/movements':
            try:
                init_db(); import urllib.parse; p=urllib.parse.parse_qs(self.path.split('?',1)[1] if '?' in self.path else ''); sku=(p.get('sku') or [''])[0]
                with db() as c,x:
                    q='SELECT id,sku,movement_type,delta,resulting_stock,reason,notes,created_by,created_at FROM mm_stock_movements '+('WHERE sku=%s ' if sku else '')+'ORDER BY created_at DESC LIMIT 500'; x.execute(q,(sku,) if sku else ()); rows=x.fetchall()
        ...
        if path.endswith('.html') or path in ('','/'):
            ...
            if os.path.isfile(fn):
                try:
                    ...
                    for i in ins:
                        if marker in data and i not in data:data=data.replace(marker,i+marker,1)
    ...
    def do_POST(self):
        ...
        if path not in ('/api/stock/ensure','/api/stock/adjust'):self.send_json(404,{'ok':False,'error':'Endpoint não encontrado'});return
        try:
            ...
            with db() as c,x:
                if path=='/api/stock/ensure':
                    ...
                    c.commit();self.send_json(200,{'ok':True});return
                ...
                x.execute('INSERT INTO mm_product_stock(sku,stock,stock_min) VALUES(%s,0,%s) ON CONFLICT(sku) DO NOTHING',(sku,minimum));x.execute('SELECT stock,stock_min FROM mm_product_stock WHERE sku=%s FOR UPDATE',(sku,));old,oldmin=map(int,x.fetchone())
                delta=qty if typ in ('in','entry','Entrada (+)','return','Devolução') else -qty if typ in ('out','exit','Saída / venda') else qty-old if typ in ('adjust','Ajuste para quantidade') else (_ for _ in ()).throw(ValueError('Tipo de movimento inválido'));new=old+delta
                if new<0:raise ValueError('O stock não pode ficar negativo')
                x.execute('UPDATE mm_product_stock SET stock=%s,stock_min=%s,updated_at=NOW() WHERE sku=%s',(new,minimum if 'stockMin' in body else oldmin,sku));x.execute('INSERT INTO mm_stock_movements(sku,movement_type,delta,resulting_stock,reason,notes,created_by) VALUES(%s,%s,%s,%s,%s,%s,%s)',(sku,typ,delta,new,str(body.get('reason') or ''),str(body.get('notes') or ''),str(body.get('user') or 'MarquesMater')));c.commit()
            self.send_json(200,{'ok':True,'sku':sku,'stock':new,'delta':delta})
        except Exception as e:self.send_json(400 if isinstance(e,ValueError) else 503,{'ok':False,'error':str(e)})
