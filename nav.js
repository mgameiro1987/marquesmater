...
(function(){
  ...
  function styles(){
    ...
    st.textContent=`
      ...
      @media (hover:hover) and (pointer:fine){.mm-nav-family:hover>.mm-nav-dropdown,.mm-nav-family:focus-within>.mm-nav-dropdown{display:block}}
      ...
      @media(max-width:1100px){.mm-footer-main-v17{grid-template-columns:1.5fr 1fr 1fr 1.35fr}.mm-footer-address-v17{grid-column:2/-1}}
      @media(max-width:900px){.mm-nav{justify-content:flex-start;overflow-x:auto!important;overflow-y:visible!important}.mm-nav-link{padding:14px 10px;font-size:11px}.menu>.container{overflow:visible!important}}
      @media(max-width:700px){.mm-footer-v17{padding:34px 0 0!important}.mm-footer-main-v17{grid-template-columns:1fr 1fr;gap:28px 20px;padding-bottom:30px}.mm-footer-brand-v17{grid-column:1/-1}.mm-footer-address-v17{grid-column:auto}.mm-footer-logo-v17{width:185px}.mm-footer-col-v17 h4{font-size:13px}.mm-footer-col-v17 a{font-size:12px!important}}
      @media(max-width:430px){.mm-footer-main-v17{grid-template-columns:1fr}.mm-footer-brand-v17,.mm-footer-address-v17{grid-column:auto}}
      /* Correção final da homepage: eliminar os cartões antigos de marcas. */
      .brands-featured .brands-grid{display:none!important}
      .brands-featured .section-link{display:none!important}
    `;
    ...
    f.innerHTML=`
      <div class="container mm-footer-main-v17">
        <div class="mm-footer-brand-v17">
          ...
          <div class="mm-footer-contact-mini-v17">${c.phone?`<span>☎ ${esc(c.phone)}</span>`:''}${c.email?`<span>✉ ${esc(c.email)}</span>`:''}</div>
        </div>
        <div class="mm-footer-col-v17"><h4>MarquesMater</h4><a href="empresa.html">Quem somos</a><a href="contactos.html">Contactos</a></div>
        <div class="mm-footer-col-v17"><h4>Informação</h4><a href="termos-condicoes.html">Termos e Condições</a><a href="politica-privacidade.html">Política de Privacidade</a><a href="politica-cookies.html">Política de Cookies</a><a href="entregas-devolucoes.html">Entregas, Trocas e Devoluções</a><a class="mm-footer-book-v17" href="livro-reclamacoes.html"><span class="mm-footer-book-icon-v17">📕</span><span>Livro de Reclamações</span></a></div>
        <div class="mm-footer-col-v17 mm-footer-address-v17"><h4>Contactos</h4><p>${esc(c.address||'')}</p>${c.postal?`<p>${esc(c.postal)}</p>`:''}${c.phone?`<a href="tel:${esc(c.phone)}">${esc(c.phone)}</a>`:''}${c.email?`<a href="mailto:${esc(c.email)}">${esc(c.email)}</a>`:''}${c.maps_url?`<a href="${esc(c.maps_url)}" target="_blank" rel="noopener">📍 Como chegar</a>`:''}</div>
      </div>
      <div class="container mm-footer-bottom-v17"><span>© ${new Date().getFullYear()} MarquesMater — Materiais de Construção e Pinturas, Lda.</span><span>Todos os direitos reservados.</span></div>`;
  }

  function bindSecret(){
    let clicks=0,timer;
    document.addEventListener('click',e=>{
      const img=e.target.closest('img.logo'); if(!img)return;
      const a=img.closest('a'); if(!a)return;
      e.preventDefault(); clicks++; clearTimeout(timer);
      if(clicks>=3){clicks=0;window.open('/admin.html','_blank','noopener');return}
      timer=setTimeout(()=>{clicks=0;location.href='index.html'},650);
    });
  }

  function bindMobileMenu(){
    document.addEventListener('click',e=>{
      const link=e.target.closest('.mm-nav-link'); if(!link)return;
      const item=link.closest('.mm-nav-family'); if(!item||!item.querySelector('.mm-nav-dropdown'))return;
      if(window.matchMedia('(max-width: 900px)').matches){document.querySelectorAll('.mm-nav-family.mm-open').forEach(x=>x.classList.remove('mm-open'))}
    });
  }

  document.addEventListener('DOMContentLoaded',()=>{
    styles(); bindSecret(); bindMobileMenu();
    fetch('/api/site',{cache:'no-store'}).then(r=>r.json()).then(site=>{render(site);footer(site)}).catch(()=>{render({families:[]});footer({settings:{}})});
  });
})();

/* Correção final: remove marcadores de conflito que possam estar fora do <html> e limpa duplicados. */
(function(){
  function clean(){
    try{
      ...
      nodes.forEach(x=>x.parentNode&&x.parentNode.removeChild(x));
      document.querySelectorAll('.brands-featured .brands-grid').forEach(x=>x.remove());
      document.querySelectorAll('.brands-featured .section-link').forEach(x=>x.remove());
    }catch(e){}
  }
