/* MarquesMater V10.6 — sincronização central + motor de promoções + estilo Front Office */
(()=>{
  ...
  async function pullCatalog(){try{
    ...
    if(r?.ok&&Array.isArray(r.catalog)&&r.catalog.length){
      const sig=JSON.stringify(r.catalog.map(p=>[p.sku,p.updated_at,p.price,p.oldPrice,p.stock,p.active,p.options?.__promo,p.category_id,p.subcategory_id,p.family_id,p.brand_id,p.attributes]));
  ...
  async function boot(){loadFrontOfficeStyle();await pullCatalog();await loadPromoEngine();await pullUser();setInterval(push,2500);window.addEventListener('storage',()=>setTimeout(push,50));window.addEventListener('mm-v10-sync',()=>setTimeout(push,50));}
