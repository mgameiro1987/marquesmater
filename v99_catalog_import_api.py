...
def _match_field(header):
    ...
    for field,keys in _ALIAS_NORM.items():
        if n in keys:
            return field
    if n.startswith('sku') or n.endswith('sku'):
        return 'sku'
    if 'ean' in n or 'barcode' in n or 'codigodebarras' in n:
        return 'barcode'
    if 'descricao' in n and ('curta' in n or 'produto' in n):
        return 'name'
    if 'descricao' in n:
        return 'description'
    if 'familia' in n:
        return 'family'
    if 'subfamilia' in n or 'subcategoria' in n:
        return 'subcategory'
    if 'categoria' in n:
        return 'category'
    if 'marca' in n:
        return 'brand'
    if 'pvp' in n or 'preco' in n:
        return 'price'
    if 'custo' in n:
        return 'cost'
    if 'iva' in n:
...
def _row_payload(raw):
    ...
    for k,v in raw.items():
        ...
        if field and field not in normalized:
            normalized[field]=v
    out=dict(normalized)
    if 'name' not in out and 'description' in out:
        out['name']=out['description']
    for k in ('sku','name','brand','category','subcategory','family','type','barcode','description','image','badge'):
        out[k]=_clean(out.get(k))
    out['price']=_num(out.get('price'))
    out['oldPrice']=_num(out.get('oldPrice'))
    out['cost']=_num(out.get('cost'))
    out['vatRate']=_num(out.get('vatRate'),23)
    out['stockMin']=max(0,int(_num(out.get('stockMin'),0)))
    out['stockInitial']=max(0,int(_num(out.get('stockInitial'),0)))
...
def _find_header(ws):
    ...
    for row_number,row in enumerate(ws.iter_rows(min_row=1,max_row=min(ws.max_row,60),values_only=True),1):
        ...
        if 'sku' in fields and ('name' in fields or 'description' in fields):
            score=3 + int('brand' in fields) + int('category' in fields) + int('price' in fields)
...
def _read_excel(data):
    ...
    candidates=[]
    preferred=('produtos','catalogo','catálogo','rida','composição skus','composicao skus')
    for ws in wb.worksheets:
        ...
        for r in range(header_row+1,ws.max_row+1):
            ...
            if not item.get('sku') and not item.get('name'):
                continue
            if r in images:
                item['image']=images[r]
            rows.append(item)
        if rows:
            ...
            preference=1 if any(p in title for p in preferred) else 0
            candidates.append((preference,len(rows),ws.title,rows))
    if not candidates:
        raise ValueError('Excel: não encontrei uma folha com cabeçalhos de produto contendo SKU e Descrição/Nome.')
...
def _extract_multipart(handler):
    ...
    if not m:
        raise ValueError('Multipart inválido: boundary em falta')
...
def _preview(rows,default_stock):
    explicit=sum(int(r.get('stockInitial',0) or 0) for r in rows)
    fallback=sum(1 for r in rows if int(r.get('stockInitial',0) or 0)==0)*default_stock
...
def _import_rows(rows,default_stock):
    ...
    errors=[]
    created=updated=images=stock_added=0
    with psycopg.connect(os.environ['DATABASE_URL']) as conn:
        with conn.cursor() as cur:
            for line,row in enumerate(rows,2):
                cur.execute('SELECT stock FROM mm_product_stock WHERE sku=%s',(row['sku'],))
                stock_row=cur.fetchone()
                had_stock=stock_row is not None
                payload={k:v for k,v in row.items() if k not in ('stockInitial','_filename')}
                captured=[]
                ...
                if status>=300 or not result.get('ok'):
                    errors.append({'linha':line,'sku':row['sku'],'error':result.get('error','Erro desconhecido')})
                    continue
                if row.get('image','').startswith('data:image/'):
                    images+=1
                if had_stock:
                    updated+=1
                else:
...
def handle_upload(handler,send_json):
    try:
        ...
        if not rows:
            raise ValueError('O ficheiro não contém linhas de dados.')
        if len(rows)>5000:
            raise ValueError('Limite de 5000 linhas por importação.')
        bad=[i+2 for i,r in enumerate(rows) if not r.get('sku') or not r.get('name')]
        if bad:
            raise ValueError('SKU e Nome são obrigatórios. Linhas inválidas: '+', '.join(map(str,bad[:20]))+('…' if len(bad)>20 else ''))
        qs=parse_qs(handler.path.split('?',1)[1] if '?' in handler.path else '')
        mode=(qs.get('mode') or [''])[0]
        default_stock=max(0,int(_num((qs.get('defaultStock') or ['0'])[0],0)))
        if mode=='preview':
            send_json(200,{'ok':True,'filename':filename,'preview':_preview(rows,default_stock)})
            return True
        for r in rows:
        ...
        send_json(503,{'ok':False,'error':f'Importação: {e}'})
        return True
