...
def _row(row):
    return {
        'id': row[0], 'sku': row[1], 'brand': row[2] or '', 'name': row[3] or '',
        'category': row[4] or '', 'subcategory': row[5] or '', 'type': row[6] or '',
        'price': float(row[7] or 0), 'oldPrice': float(row[8] or 0), 'stockText': row[9] or '',
        'image': row[10] or '', 'badge': row[11] or '', 'description': row[12] or '',
        'options': row[13] or [], 'specs': row[14] or {}, 'active': bool(row[15]),
        'createdAt': row[16], 'updatedAt': row[17], 'categoryId': row[18],
        'subcategoryId': row[19], 'familyId': row[20], 'brandId': row[21],
        'attributes': row[22] or {}, 'barcode': row[23] or ''
    }


SELECT="""SELECT p.id,p.sku,p.brand,p.name,p.category,p.subcategory,p.type,p.price,p.old_price,p.stock,p.image,p.badge,p.description,p.options,p.specs,p.active,p.created_at,p.updated_at,p.category_id,p.subcategory_id,p.family_id,p.brand_id,p.attributes,p.barcode,
COALESCE(s.stock,0) AS real_stock,COALESCE(s.stock_min,0) AS stock_min,s.updated_at AS stock_updated_at
FROM catalog_products p LEFT JOIN mm_product_stock s ON s.sku=p.sku"""
...
def handle_get(path, query, send_json):
    ...
    try:
        ...
        with db() as conn, conn.cursor() as cur:
            if sku:
                cur.execute(SELECT+' WHERE p.sku=%s LIMIT 1',(sku,))
                row=cur.fetchone()
                if not row: send_json(404,{'ok':False,'error':'Produto não encontrado'}); return True
                item=_row(row); item.update({'stock':int(row[24] or 0),'stockMin':int(row[25] or 0),'stockUpdatedAt':row[26]})
                send_json(200,{'ok':True,'item':item}); return True
            cur.execute(SELECT+' ORDER BY p.name ASC,p.id ASC')
            items=[]
        ...
        send_json(503,{'ok':False,'error':f'API produtos: {e}'}); return True
...
def handle_post(path, body, send_json):
    ...
    try:
        ...
        price=_num(body.get('price')); old_price=_num(body.get('oldPrice'))
        active=bool(body.get('active',True))
        stock_text=_clean(body.get('stockText'))
        ...
        fields=(sku,_clean(body.get('brand')),name,_clean(body.get('category')),_clean(body.get('subcategory')),_clean(body.get('type')),
                price,old_price,stock_text,_clean(body.get('image')),_clean(body.get('badge')),_clean(body.get('description')),
                json.dumps(options,ensure_ascii=False),json.dumps(specs,ensure_ascii=False),active,_int_or_none(body.get('categoryId')),
                _int_or_none(body.get('subcategoryId')),_int_or_none(body.get('familyId')),_int_or_none(body.get('brandId')),
                json.dumps(attributes,ensure_ascii=False),barcode)
        with db() as conn, conn.cursor() as cur:
            ...
            if cur.fetchone() is None:
                cur.execute('INSERT INTO mm_product_stock(sku,stock,stock_min) VALUES(%s,%s,%s)',(sku,max(0,int(_num(body.get('stock'),0))),max(0,int(_num(body.get('stockMin'),0)))))
            else:
                cur.execute('UPDATE mm_product_stock SET stock_min=%s,updated_at=NOW() WHERE sku=%s',(max(0,int(_num(body.get('stockMin'),0))),sku))
            conn.commit()
        send_json(200,{'ok':True,'id':pid,'sku':sku}); return True
    except ValueError as e:
        send_json(400,{'ok':False,'error':str(e)}); return True
    except Exception as e:
        send_json(409 if 'duplicate' in str(e).lower() else 503,{'ok':False,'error':f'Não foi possível guardar o produto: {e}'}); return True
