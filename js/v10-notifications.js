(function(){
  ...
  function loadData(){
    ...
    /* As encomendas estão atualmente associadas ao cliente demo usado no checkout.
       O endpoint central exige email; usamos o mesmo email demo para o painel. */
    var ordersPromise=fetch('/api/orders',{cache:'no-store',headers:{'X-Customer-Email':'demo@marquesmater.pt'}}).then(function(r){return r.ok?r.json():{}}).catch(function(){return {}});
    return Promise.all([catalogPromise,ordersPromise]).then(function(res){
      ...
      var pending=orders.filter(function(o){var d=o&&o.data&&typeof o.data==='object'?o.data:o;var s=String(d.status||'').toLowerCase();return !/conclu|cancel|entregue|levant/.test(s)});
      if(pending.length){
        var latest=pending[0],d=latest&&latest.data&&typeof latest.data==='object'?latest.data:latest;
        var num=d.number||('#'+(latest.id||''));
        var total=d.total!=null?Number(d.total).toFixed(2).replace('.',',')+' €':'';
        var customer=d.customer&&d.customer.name?d.customer.name:(d.name||'Cliente Demo');
        items.push({type:'info',icon:'🛒',title:pending.length===1?'Nova encomenda recebida':pending.length+' encomendas por tratar',text:num+' · '+customer+(total?' · '+total:''),section:'orders'});
      }
