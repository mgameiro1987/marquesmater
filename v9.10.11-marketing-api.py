...
def post(body,send_json):
    ...
    with _db() as c, c.cursor() as x:
        if kind=='coupon':
            ...
            typ=str(body.get('discountType') or 'percent')
...
def patch(body,send_json):
    ensure(); kind=str(body.get('kind') or 'coupon'); ident=int(body.get('id') or 0); active=bool(body.get('active'))
    table='mm_marketing_coupons' if kind=='coupon' else 'mm_marketing_promotions'
    with _db() as c, c.cursor() as x:
