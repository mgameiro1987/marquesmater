...
def _items_subtotal(items):
    ...
    for item in items:
        try:
            ...
            raise ValueError('Linha de artigo inválida')
        total += price * qty
...
def create_order(body, send_json):
    ...
    if not isinstance(items, list) or not items:
        raise ValueError('A encomenda tem de conter pelo menos um artigo.')

    # O subtotal enviado pelo browser nunca é a fonte de verdade: é recalculado
    # a partir das linhas da encomenda para evitar que o total fique apenas com portes.
    subtotal = _items_subtotal(items)
    if subtotal <= 0:
        raise ValueError('O subtotal tem de ser superior a 0 €.')
    ...
    if final_total <= 0:
        raise ValueError('Uma encomenda não pode ter custo zero ou negativo.')

    clean_body = dict(body)
    clean_body['subtotal'] = subtotal
    ...
    if not handled:
        raise RuntimeError('Não foi possível registar a encomenda.')
        ...
        if oid:
            with _db() as conn, conn.cursor() as cur:
                marketing = {
                    ...
                    'couponDiscount': result.get('couponDiscount', 0),
                    'promotionDiscount': result.get('promotionDiscount', 0),
                ...
                cur.execute('SELECT data FROM orders WHERE id=%s FOR UPDATE', (int(oid),))
                row = cur.fetchone()
                if row:
                    data = row[0] or {}
                    data['marketing'] = marketing
                    data['subtotal'] = subtotal
                    data['deliveryCost'] = delivery_cost
                ...
                if result.get('coupon'):
                    consume_coupon(cur, result['coupon'])
                conn.commit()
    return True
