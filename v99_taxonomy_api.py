...
def ensure(cur):
    ...
    for (name,) in cur.fetchall(): cur.execute("INSERT INTO mm_categories(name) VALUES(%s) ON CONFLICT(name) DO NOTHING",(name,))
    ...
    for c,s in cur.fetchall(): cur.execute("INSERT INTO mm_subcategories(category_id,name) VALUES(%s,%s) ON CONFLICT(category_id,name) DO NOTHING",(cats[c],s))
    ...
    for c,s,f in cur.fetchall():
        sid=None
        if s:
            cur.execute("SELECT id FROM mm_subcategories WHERE category_id=%s AND name=%s",(cats[c],s)); z=cur.fetchone(); sid=z[0] if z else None
        cur.execute("INSERT INTO mm_families(category_id,subcategory_id,name) VALUES(%s,%s,%s) ON CONFLICT(category_id,name) DO UPDATE SET subcategory_id=COALESCE(EXCLUDED.subcategory_id,mm_families.subcategory_id)",(cats[c],sid,f))
...
def handle_get(path,query,send_json):
    ...
    except Exception as e: send_json(503,{'ok':False,'error':f'API taxonomia: {e}'});return True

def handle_post(path,body,send_json):
    if path!='/api/taxonomy': return False
    try:
        kind=clean(body.get('kind'));name=clean(body.get('name'));cid=int(body.get('categoryId') or 0) if str(body.get('categoryId') or '').isdigit() else 0;sid=int(body.get('subcategoryId') or 0) if str(body.get('subcategoryId') or '').isdigit() else None
...
def handle_patch(path,body,send_json):
    ...
    try:
        ...
        with db() as conn,conn.cursor() as cur:
            ...
            table={'category':'mm_categories','subcategory':'mm_subcategories','family':'mm_families'}[kind]
            if kind=='category': cur.execute("UPDATE mm_categories SET name=%s,description=%s,icon=%s,active=%s,updated_at=NOW() WHERE id=%s",(name,clean(body.get('description')),clean(body.get('icon')),active,rid))
            elif kind=='subcategory': cur.execute("UPDATE mm_subcategories SET name=%s,active=%s,updated_at=NOW() WHERE id=%s",(name,active,rid))
            else: cur.execute("UPDATE mm_families SET name=%s,subcategory_id=%s,active=%s,updated_at=NOW() WHERE id=%s",(name,int(body.get('subcategoryId')) if str(body.get('subcategoryId') or '').isdigit() else None,active,rid))
            if cur.rowcount!=1: raise ValueError('Registo não encontrado')
            conn.commit();send_json(200,{'ok':True});return True
    except psycopg.errors.UniqueViolation: send_json(409,{'ok':False,'error':'Já existe um registo com esse nome neste nível.'});return True
    except ValueError as e: send_json(400,{'ok':False,'error':str(e)});return True
    except Exception as e: send_json(503,{'ok':False,'error':f'API taxonomia: {e}'});return True
