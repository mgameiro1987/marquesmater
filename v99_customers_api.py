...
def _orders_for(cur, cid, email):
    ...
    for oid,created,data in cur.fetchall():
        data=data or {}; total=_money(data.get('total'))
        if total<=0: continue
...
def _spent(orders):
    return round(sum(x['total'] for x in orders if str(x.get('status') or '').strip().lower()!='cancelada'),2)
...
def handle_get(path, query, send_json):
    try:
        ...
        with _db() as conn, conn.cursor() as cur:
            ...
            items=[]
            for d in rows:
                cid=d.get('id'); email=str(d.get('email') or '').strip(); orders=_orders_for(cur,cid,email)
                items.append({'id':cid,'name':d.get('name') or '','email':email,'phone':d.get('phone') or '','created_at':d.get('created_at'),'updated_at':d.get('updated_at'),'orders_count':len(orders),'total_spent':_spent(orders),'last_order':orders[0]['created_at'] if orders else None})
            ...
            if detail:
                ...
                send_json(200,{'ok':True,'item':{'id':found.get('id'),'name':found.get('name') or '','email':email,'phone':found.get('phone') or '','created_at':found.get('created_at'),'updated_at':found.get('updated_at'),'orders_count':len(orders),'total_spent':_spent(orders),'orders':orders}}); return True
...
def handle_post(path, body, send_json):
    ...
    try:
        with _db() as conn, conn.cursor() as cur:
            ...
            if old: send_json(409,{'ok':False,'error':'Já existe um cliente com este email','id':old[0]}); return True
