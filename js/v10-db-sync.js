/* MarquesMater V10.1 — sincronização central: catálogo, conta, carrinho e favoritos. */
(()=>{
  ...
  async function pullCatalog(){try{
    ...
    if(r?.ok&&Array.isArray(r.catalog)&&r.catalog.length){
      const sig=JSON.stringify(r.catalog.map(p=>[p.sku,p.updated_at,p.price,p.stock,p.active]));
  ...
  async function boot(){await pullCatalog();await pullUser();setInterval(push,2500);window.addEventListener('storage',()=>setTimeout(push,50));window.addEventListener('mm-v10-sync',()=>setTimeout(push,50));}
